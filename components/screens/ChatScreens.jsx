...
import { ArrowLeft, MessageSquare, Send, Package } from "lucide-react";
...
export function ChatListScreen() {
  ...
  return (
    <div className="chat-list h-full flex flex-col bg-card">
      <div className="p-5 border-b border-border">
        <h1 className="text-xl font-extrabold text-foreground">채팅</h1>
        <p className="text-sm text-muted-foreground mt-0.5">{rooms.length}개의 대화</p>
      </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {rooms.map((r) => (
            <button
              ...
              className={`w-full p-4 flex items-start gap-3 hover:bg-muted transition-colors text-left border-b border-border/50 ${
                r.roomId === activeRoomId ? "bg-amber/5 border-l-2 border-l-amber" : ""
              }`}
            >
              <Avatar name={r.partnerNickName || "?"} size={44} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span className="font-semibold text-foreground text-sm truncate">{r.partnerNickName || "상대"}</span>
                  <span className="text-xs text-muted-foreground flex-shrink-0 ml-2">{timeLabel(r.lastMessageAt)}</span>
                </div>
                <p className="text-xs text-muted-foreground truncate mb-1">{r.foodName}</p>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm text-muted-foreground truncate flex-1">{r.lastMessage || "대화를 시작해보세요"}</p>
                  {r.unreadCount > 0 && (
                    <span className="bg-amber text-white rounded-full min-w-[20px] h-5 px-1.5 text-xs flex items-center justify-center flex-shrink-0">
                      {r.unreadCount}
                    </span>
                  )}
                </div>
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center text-center p-8">
      <div className="w-20 h-20 rounded-3xl bg-amber/10 flex items-center justify-center mb-5">
        <MessageSquare className="w-10 h-10 text-amber" />
      </div>
      <h3 className="text-xl font-bold text-foreground mb-2">채팅을 선택해주세요</h3>
      <p className="text-muted-foreground text-sm max-w-xs">
        왼쪽 목록에서 대화를 선택하거나,<br />물품 상세 페이지에서 채팅을 시작하세요.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border bg-card">
        <button
          className="md:hidden p-1.5 rounded-lg hover:bg-muted transition-colors text-foreground"
          onClick={() => router.push("/chat")}
          aria-label="뒤로"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <Avatar name={room ? (room.partnerNickName || "상대") : "?"} size={36} />
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-foreground text-sm truncate">{room ? (room.partnerNickName || "상대") : "채팅"}</p>
          {room && (
            <button
            ...
            >
              ...
              <span className="text-xs text-muted-foreground truncate">{room.foodName}</span>
            </button>
          )}
        </div>
        <span
          className={`text-xs font-semibold whitespace-nowrap ${
            status === "connected" ? "text-green-600" : status === "connecting" ? "text-muted-foreground" : "text-destructive"
          }`}
        >
      ...
      >
        {loading ? (
          ...
          <>
            {loadingMore && (
              <div className="flex justify-center pb-1"><Spinner size={20} /></div>
            )}
            {messages.length === 0 && (
            ...
            {messages.map((m, i) => (
              <Fragment key={m.messageId ?? i}>
                {i === firstUnreadIdx && (
                  <div data-new-divider className="flex items-center gap-3 text-amber text-xs font-bold my-1">
                ...
                <div className={`flex items-end gap-2 ${m.mine ? "flex-row-reverse" : ""}`}>
                  {!m.mine && <Avatar name={m.senderNickName || "?"} size={32} />}
                  <div className={`max-w-[70%] flex flex-col gap-1 ${m.mine ? "items-end" : "items-start"}`}>
                    {!m.mine && <span className="text-xs text-muted-foreground px-1">{m.senderNickName}</span>}
                    <div
                    ...
                    <span className="text-xs text-muted-foreground px-1">{timeLabel(m.createdAt)}</span>
                  </div>
                </div>
              </Fragment>
            ))}
      ...
      <div className="p-4 border-t border-border bg-card">
        <div className="flex gap-2 items-end">
          <input
            ...
            disabled={loading}
            className="flex-1 h-11 px-4 rounded-xl bg-cream border border-border text-sm text-foreground focus:border-amber focus:outline-none"
          />
          <button
            onClick={send}
            disabled={!input.trim() || status !== "connected"}
            aria-label="보내기"
            className="w-11 h-11 rounded-xl bg-amber text-white flex items-center justify-center flex-shrink-0 disabled:opacity-50"
          >
