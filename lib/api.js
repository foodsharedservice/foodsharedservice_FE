...
const API = {
  ...
  foods: {
    ...
    // PATCH /foods/{foodId} { foodName, capacity, details } (JSON) → null
    //   수정 화면에는 사진 입력이 없어 JSON 본문으로 보낸다(세 필드 모두 필수).
    //   expiredImageId(소비기한 사진 교체) / deleteImageIds(기존 이미지 삭제)는 선택.
    update: (foodId, { foodName, capacity, details, expiredImageId, deleteImageIds } = {}) => {
      const body = { foodName, capacity, details };
      if (expiredImageId !== undefined) body.expiredImageId = expiredImageId;
      if (deleteImageIds !== undefined) body.deleteImageIds = deleteImageIds;
