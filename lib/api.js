...
const API = {
  ...
  foods: {
    ...
    // → { expiredImageId, accessUrl, expired }   (expired는 AI 인식값, 수정불가)
    ...
    // POST /foods  (D-04 등록) — multipart: request(JSON) + images[]
    create: ({ foodName, expiredImageId, capacity, details, images = [] }) => {
      const fd = new FormData();
      fd.append(
        "request",
        new Blob(
          [JSON.stringify({ foodName, expiredImageId, capacity, details })],
